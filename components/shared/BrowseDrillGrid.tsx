...
export interface BrowseDrillEntry {
  key: string;
  label: string;
  count: number;
}

export interface BrowseDrillViewAllLead {
  count: number;
  onClick: () => void;
}

interface Props {
  entries: BrowseDrillEntry[];
  onSelect: (key: string) => void;
  /** Dòng đếm dưới mỗi thẻ (vd. `t('…browse.recordCount', { count })`) */
  formatRecordCount: (count: number) => string;
  viewAllLead?: BrowseDrillViewAllLead | null;
}

const BrowseDrillGrid: React.FC<Props> = ({ entries, onSelect, formatRecordCount, viewAllLead }) => {
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 list-none p-0 m-0">
        ...
        {entries.map((e) => (
          <li key={e.key}>
            ...
            >
              <span className="text-sm font-medium text-foreground line-clamp-3">{e.label}</span>
              <span className="text-xs tabular-nums text-muted-foreground">{formatRecordCount(e.count)}</span>
            </button>
          </li>
        ))}
      </ul>
...
export default BrowseDrillGrid;
