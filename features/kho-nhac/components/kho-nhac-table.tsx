...
import { Edit, Trash2, Music2, Link2 } from 'lucide-react';
...
interface Props {
  ...
  onView: (item: KhoNhac) => void;
  canUpdate?: boolean;
  canDelete?: boolean;
}

const KhoNhacTable: React.FC<Props> = ({
  data,
  isLoading,
  onEdit,
  onDelete,
  ...
  const renderCell = (colId: string, item: KhoNhac) => {
    switch (colId) {
      case 'bo_suu_tap':
        return (
      ...
      case 'ten_nhac':
        return (
          <div className="flex items-start gap-2 min-w-0">
            <Music2 size={14} className="text-primary shrink-0 mt-0.5" aria-hidden />
            <div className="flex flex-col min-w-0">
              <span className="font-semibold text-foreground text-sm line-clamp-2">{item.ten_nhac}</span>
  ...
  const renderMobileCard = (item: KhoNhac, isSelected: boolean) => (
    <div
      key={item.id}
      onClick={() => onView(item)}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h4 className="font-semibold text-foreground text-sm line-clamp-2 flex-1">{item.ten_nhac}</h4>
        <input
          type="checkbox"
          checked={isSelected}
      ...
      <p className="text-xs text-muted-foreground mb-2">{item.bo_suu_tap || '—'}</p>
      {canUpdate || canDelete ? (
  ...
  return (
    <GenericTable
      ...
      stickyLeftCount={1}
      emptyTitle={t('khoNhac.dm.table.empty')}
    />
  );
};

export default KhoNhacTable;
