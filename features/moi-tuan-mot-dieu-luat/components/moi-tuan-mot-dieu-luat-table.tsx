...
const MoiTuanMotDieuLuatTable: React.FC<Props> = ({
  ...
  const renderMobileCard = (item: MoiTuanMotDieuLuat, isSelected: boolean) => (
    <div
      key={item.id}
      onClick={() => onView(item)}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h4 className="font-semibold text-foreground text-sm line-clamp-2 flex-1">{item.ten_dieu_luat}</h4>
        <input
          type="checkbox"
          checked={isSelected}
      ...
      <p className="text-xs text-muted-foreground mb-1">{item.nam_thang_tuan || '—'}</p>
      {canUpdate || canDelete ? (
