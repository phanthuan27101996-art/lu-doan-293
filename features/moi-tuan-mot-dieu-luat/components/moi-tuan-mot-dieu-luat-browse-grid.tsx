...
export interface MoiTuanBrowseGridEntry {
  key: string;
  label: string;
  count: number;
}

export interface MoiTuanViewAllLead {
  count: number;
  onClick: () => void;
}

interface Props {
  entries: MoiTuanBrowseGridEntry[];
  onSelect: (key: string) => void;
  viewAllLead?: MoiTuanViewAllLead | null;
}

/** Lưới drill: năm hoặc tháng (yyyy/mm) — không tiêu đề. */
const MoiTuanMotDieuLuatBrowseGrid: React.FC<Props> = ({ entries, onSelect, viewAllLead }) => {
        ...
        {entries.map((e) => (
          <li key={e.key}>
            ...
            >
              ...
              <span className="text-xs tabular-nums text-muted-foreground">
                {t('moiTuanMotDieuLuat.dm.browse.recordCount', { count: e.count })}
              </span>
            </button>
          </li>
        ))}
...
export default MoiTuanMotDieuLuatBrowseGrid;
