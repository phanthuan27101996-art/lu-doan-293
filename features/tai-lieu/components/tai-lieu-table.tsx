...
import { Edit, Trash2, FileText, Link2 } from 'lucide-react';
...
const TaiLieuTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
      ...
      case 'nhom_tai_lieu':
        return (
          <span className="text-body-sm font-medium text-foreground truncate block max-w-[180px]">
            {item.nhom_tai_lieu?.trim() || '—'}
          </span>
        );
      case 'ten_tai_lieu':
        return (
          <div className="flex items-start gap-2 min-w-0">
            <FileText size={14} className="text-primary shrink-0 mt-0.5" aria-hidden />
  ...
  const renderMobileCard = (item: TaiLieu, isSelected: boolean) => (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground mb-2">{item.nhom_tai_lieu || '—'}</p>
  ...
  return (
    <GenericTable
      ...
      stickyLeftCount={1}
      emptyTitle={t('taiLieu.dm.table.empty')}
    />
  );
};
