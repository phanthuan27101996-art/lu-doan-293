...
export interface TaiLieuBrowseEntry {
  key: string;
  label: string;
  count: number;
}

export interface TaiLieuViewAllLead {
  count: number;
  onClick: () => void;
}

interface Props {
  entries: TaiLieuBrowseEntry[];
  onSelect: (key: string) => void;
  viewAllLead?: TaiLieuViewAllLead | null;
}

const TaiLieuBrowseLevels: React.FC<Props> = ({ entries, onSelect, viewAllLead }) => {
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 list-none p-0 m-0">
        ...
        {entries.map((e) => (
          <li key={e.key}>
            ...
            >
              <span className="text-sm font-medium text-foreground line-clamp-2">{e.label}</span>
              <span className="text-xs tabular-nums text-muted-foreground">
                {t('taiLieu.dm.browse.docCount', { count: e.count })}
              </span>
            </button>
          </li>
        ))}
...
export default TaiLieuBrowseLevels;
