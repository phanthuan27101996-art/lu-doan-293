...
const TaiLieuDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete }) => {
  ...
  return (
    <GenericDrawer
      title={t('taiLieu.dm.detail.title')}
      subtitle={`id ${data.id}`}
    ...
    >
      <div className="space-y-5">
        <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm">
          <p className="text-xs text-muted-foreground mb-1">{data.nhom_tai_lieu || '—'}</p>
          ...
          <DetailFieldGrid>
            ...
            {data.link ? (
              <div className="col-span-full">
                <p className="text-xs font-medium text-muted-foreground mb-1">{t('taiLieu.dm.form.link')}</p>
                <a
                  href={data.link}
                  target="_blank"
              ...
              <DetailField label={t('taiLieu.dm.form.link')} value="—" />
            )}
            <DetailField label={t('taiLieu.dm.form.ghiChu')} value={data.ghi_chu?.trim() ? data.ghi_chu : '—'} />
            <DetailField
              label={t('taiLieu.dm.detail.nguoiTao')}
              value={
                data.id_nguoi_tao
            ...
            />
            <DetailField label={t('taiLieu.dm.detail.createdAt')} value={data.tg_tao ? formatDate(data.tg_tao) : '—'} />
            <DetailField label={t('taiLieu.dm.detail.updatedAt')} value={data.tg_cap_nhat ? formatDate(data.tg_cap_nhat) : '—'} />
          </DetailFieldGrid>
        </DetailSection>
      </div>
    </GenericDrawer>
