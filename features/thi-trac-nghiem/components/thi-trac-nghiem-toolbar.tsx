...
const ThiTracNghiemToolbar: React.FC<Props> = ({
  items,
  onAdd,
  onExport,
  onImport,
  ...
  const nhomOptions = useMemo(() => {
    ...
    return Object.keys(counts)
      ...
      .map((value) => ({
        label: value === '__empty__' ? t('thiTracNghiem.dm.toolbar.nhomEmpty') : value,
        value,
  ...
  const renderFilters = (
    <FilterChipMultiSelect
      ...
      icon={FolderOpen}
      placeholder={t('thiTracNghiem.dm.toolbar.nhom')}
  ...
  const filterGroups = useMemo(
    () => [
      {
        key: 'nhom',
        label: t('thiTracNghiem.dm.toolbar.nhom'),
        icon: FolderOpen,
        options: nhomOptions,
        value: filters.nhom,
  ...
  const mobileActions = useMemo(() => {
    ...
    if (canImport) {
      list.push({
        key: 'import',
        label: t('thiTracNghiem.dm.toolbar.importData'),
        icon: Upload,
        onClick: onImport,
        description: t('thiTracNghiem.dm.toolbar.importDesc'),
      });
    }
    if (canExport) {
      list.push({
        key: 'export',
        label: t('thiTracNghiem.dm.toolbar.exportData'),
        icon: Download,
        onClick: onExport,
        description: t('thiTracNghiem.dm.toolbar.exportDesc'),
      });
    }
    return list;
  ...
  const renderActions = (
    <>
      {canImport ? (
        <Tooltip content={t('thiTracNghiem.dm.toolbar.importData')} placement="bottom">
          <Button
            variant="outline"
            size="sm"
            onClick={onImport}
      ...
      {canExport ? (
        <Tooltip content={t('thiTracNghiem.dm.toolbar.exportData')} placement="bottom">
          <Button
            variant="outline"
            size="sm"
            onClick={onExport}
...
export default ThiTracNghiemToolbar;
