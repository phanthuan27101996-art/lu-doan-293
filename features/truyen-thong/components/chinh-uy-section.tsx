...
import { useChinhUyList, useDeleteChinhUy } from '../hooks/use-chinh-uy';
import type { ChinhUy } from '../core/chinh-uy-types';
import { nextThuTuFromChinhUyList } from '../utils/chinh-uy-form';
import { resolveTruyenThongImageSrc } from '../utils/resolve-truyen-thong-image';
import ChinhUyForm from './chinh-uy-form';
import ChinhUyDetail from './chinh-uy-detail';
...
const ChinhUySection: React.FC<SectionProps> = ({ canCreate, canUpdate, canDelete }) => {
  const { t } = useTranslation();
  const { data: items = [], isLoading, isError, error } = useChinhUyList();
  const deleteMutation = useDeleteChinhUy();
  const confirm = useConfirmStore((s) => s.confirm);

  const [formOpen, setFormOpen] = useState(false);
  const [formOrigin, setFormOrigin] = useState<FormOrigin>('list');
  const [editing, setEditing] = useState<ChinhUy | null>(null);
  const [viewing, setViewing] = useState<ChinhUy | null>(null);

  const defaultThuTu = useMemo(() => nextThuTuFromChinhUyList(items), [items]);
  ...
  const openEditFromDetail = (row: ChinhUy) => {
  ...
  const handleDelete = (id: string) => {
    ...
    confirm({
      title: t('truyenThong.chinhUy.deleteTitle'),
      message: t('truyenThong.chinhUy.deleteMessage', { name }),
      variant: 'danger',
  ...
  const galleryLabel = t('truyenThong.chinhUy.sectionTitle');

  return (
    <>
      <DetailSection
        title={t('truyenThong.chinhUy.sectionTitle')}
      ...
      >
        {isError ? (
          <p className="text-sm text-destructive" role="alert">
            {(error as Error)?.message ?? t('truyenThong.chinhUy.loadError')}
          </p>
        ) : null}
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">{t('truyenThong.chinhUy.empty')}</p>
        ) : (
