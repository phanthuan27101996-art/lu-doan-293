...
import { Shield, Edit, Trash2, ListOrdered, StickyNote, Calendar } from 'lucide-react';
...
interface Props {
  data: ChinhUy;
  onClose: () => void;
  onEdit: (item: ChinhUy) => void;
  onDelete: (id: string) => void;
  canUpdate?: boolean;
  canDelete?: boolean;
}

const ChinhUyDetail: React.FC<Props> = ({
  data,
  onClose,
  onEdit,
  onDelete,
  ...
  return (
    <GenericDrawer
      title={t('truyenThong.chinhUy.detail.title')}
    ...
    >
      <div className="space-y-5">
        ...
        <DetailSection title={t('truyenThong.chinhUy.detail.basic')} icon={<ListOrdered size={14} aria-hidden />} variant="primary">
          <DetailFieldGrid>
            <DetailField label={t('truyenThong.chinhUy.field.hoVaTen')} value={data.ho_va_ten} />
            <DetailField
              label={t('truyenThong.chinhUy.field.namSinh')}
              value={data.nam_sinh != null ? String(data.nam_sinh) : '—'}
            />
            <DetailField label={t('truyenThong.chinhUy.field.thoiGianDamNhiem')} value={period} />
            <DetailField label={t('truyenThong.chinhUy.field.chucVuCuoiCung')} value={roleLine} />
            <DetailField label={t('truyenThong.chinhUy.field.capBac')} value={data.cap_bac_hien_tai?.trim() ? data.cap_bac_hien_tai : '—'} />
          </DetailFieldGrid>
        </DetailSection>

        {(data.ghi_chu ?? '').trim() ? (
          <DetailSection title={t('truyenThong.chinhUy.field.ghiChu')} icon={<StickyNote size={14} aria-hidden />} variant="muted">
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{data.ghi_chu}</p>
          </DetailSection>
        ) : null}

        <DetailSection title={t('truyenThong.chinhUy.detail.meta')} icon={<Calendar size={14} aria-hidden />} variant="muted">
          <DetailFieldGrid>
            <DetailField
