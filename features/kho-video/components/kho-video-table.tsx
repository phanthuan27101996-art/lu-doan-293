...
import { Edit, Trash2, Video, Link2 } from 'lucide-react';
...
const KhoVideoTable: React.FC<Props> = ({
  ...
  const renderMobileCard = (item: KhoVideo, isSelected: boolean) => (
    <div
      key={item.id}
      onClick={() => onView(item)}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h4 className="font-semibold text-foreground text-sm line-clamp-2 flex-1">{item.ten_video}</h4>
        <input
          type="checkbox"
          checked={isSelected}
