...
const DoanCoSoTable: React.FC<Props> = ({
  ...
  const renderMobileCard = (item: DoanCoSo, isSelected: boolean) => (
    <div
      key={item.id}
      onClick={() => onView(item)}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h4 className="font-semibold text-foreground text-sm line-clamp-2 flex-1">{item.ten}</h4>
        <input
          type="checkbox"
          checked={isSelected}
      ...
      <p className="text-xs text-muted-foreground mb-1">{item.nhom || '—'}</p>
      <p className="text-xs text-muted-foreground mb-2">{formatDate(item.ngay)}</p>
